...
def generate_mixed_precision_cg() -> str:
    ...
    with e.indent():
        ...
        e.line("Ax = torch.zeros_like(r)")
        e.line("spmv_fn(x_lp, Ax)")
        e.line("r = rhs_lp - Ax")
        e.line("p = r.clone()")
        e.line("Ap = torch.zeros_like(r)")
        e.line("buf = torch.zeros(1, dtype=r.dtype, device=r.device)")
